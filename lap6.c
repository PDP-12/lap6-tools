...
static int directory_block = 0426;  //LINC-8 326
static word_t directory[2 * BLOCK_WORDS];
static word_t input[100 * 2 * BLOCK_WORDS];
...
{
  long offset = 2 * BLOCK_WORDS * (block - forward_offset);
...
int metadata(int *block_size, int *forward_offset, int *reverse_offset)
{
  ...
  if (size == TAPE_BLOCKS * BLOCK_WORDS * 2) {
    ...
    *forward_offset = 0;
    *reverse_offset = 0;
  } else if ((size % (2 * BLOCK_WORDS)) == 6) {
    /* Extended image with additional meta data. */
    word_t data[3];
    if (tape_seek(size / (2 * BLOCK_WORDS)) == -1)
